...
Task("BuildDeploymentProject")
  .Does(() => {
    BuildCSharpProject(
      File($"{DeployProjectFolder}/TDAshworth.PowerNotify.Deployment.csproj"), 
...
string GetConnectionString(string solution, bool stagingEnvironment) {
  ...
  var username = solutionConfig["username"] ?? EnvironmentVariable("CAKE_POWERNOTIFY_USERNAME");
  var password = EnvironmentVariable("CAKE_POWERNOTIFY_PASSWORD");
